            ...
            {sections.map((section, index) => (
              <div key={index} className="relative group">
                ...
                <button
                  ...
                  className={cn(
                    "relative w-2 h-2 sm:w-2.5 sm:h-2.5 lg:w-3 lg:h-3 rounded-full border-2 transition-all duration-300 hover:scale-150",
                    "before:absolute before:inset-0 before:rounded-full before:transition-all before:duration-300",
                    "hover:backdrop-blur-xl hover:shadow-[0_0_20px_rgba(6,182,212,0.6)] hover:shadow-cyan-500/60 hover:animate-neon-pulse",
                    activeSection === index
                      ? "bg-primary border-primary shadow-lg before:animate-ping before:bg-primary/20"
                      : "bg-background/20 backdrop-blur-md border-muted-foreground/40 hover:border-cyan-400/80 hover:bg-gradient-to-r hover:from-cyan-500/20 hover:to-purple-500/20"
                  )}
                ...
                <div className="grid gap-3 sm:gap-4 mb-8 sm:mb-10">
                  {section.features.map((feature, featureIndex) => (
                    <div
                      key={feature.title}
                      className={cn(
                        "group p-4 sm:p-5 lg:p-6 rounded-lg sm:rounded-xl border bg-card/30 backdrop-blur-xl hover:bg-card/10 hover:backdrop-blur-2xl transition-all duration-300 hover:shadow-2xl hover:shadow-purple-500/20",
                        "hover:border-purple-400/60 hover:-translate-y-1 hover:scale-[1.02] hover:ring-2 hover:ring-purple-500/30 hover:animate-border-flow"
                      )}
                ...
                <div className={cn(
                  ...
                  {section.actions.map((action, actionIndex) => (
                    <button
                      ...
                      className={cn(
                        "group relative px-6 sm:px-8 py-3 sm:py-4 rounded-lg sm:rounded-xl font-medium transition-all duration-300 hover:scale-[1.05] active:scale-[0.98] text-sm sm:text-base",
                        "hover:shadow-2xl focus:outline-none focus:ring-2 focus:ring-pink-500/30 w-full sm:w-auto overflow-hidden",
                        action.variant === 'primary'
                          ? "bg-primary/80 backdrop-blur-xl text-primary-foreground shadow-lg shadow-primary/20 hover:shadow-pink-500/50 border border-primary/30 hover:border-pink-400/60 hover:animate-glow-rotate"
                          : "border-2 border-border/60 bg-background/20 backdrop-blur-xl hover:bg-background/10 hover:backdrop-blur-2xl hover:border-cyan-400/60 hover:shadow-cyan-500/30 text-foreground hover:animate-shimmer"
                      )}
                      style={{ animationDelay: `${actionIndex * 0.1 + 0.2}s` }}
                    >
                      <span className="relative z-10">{action.label}</span>
                      {action.variant === 'primary' && (
                        <div className="absolute inset-0 rounded-lg sm:rounded-xl bg-gradient-to-r from-pink-500/40 via-purple-500/30 to-cyan-500/20 backdrop-blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 animate-shimmer" />
                      )}
                    </button>
                  ))}
                </div>
