 ...
 * Export payment transactions data to CSV format
 * Requires admin role
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (filters.status && !['success', 'failed', 'settled'].includes(filters.status)) {
      return errors.badRequest('Invalid status filter. Must be "success", "failed", or "settled"');
    }
    ...
    // Generate CSV export
    const csvData = await exportService.exportTransactions(
      processedFilters,
      authResult.user.id
    );
